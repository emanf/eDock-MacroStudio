...
def safe_locate_feature_center_on_screen(
    ...
    try:
        ...
        inlier_ratio = normalize_match_score(
            float(inlier_count)
            / float(max(1, len(good_matches)))
        )
        ...
        if (
            ...
            return failed_match_result(inlier_ratio)
...
def wait_for_captured_image(
    ...
    while True:
        ...
        if (
            ...
            return convert_result_position(
                result,
                image_value,
                position_type,
            )
...
class WaitForCapturedImageCommand(
    MacroCommand
):
    id = (
        "screen_recognition."
        "wait_for_captured_image"
    )
    title = "Wait For Captured Image"
    category = ScreenRecognitionCategory
    icon = "mc:e1bc"
    description = (
        "Wait for a captured image to appear on "
        "screen and return its position."
    )
    fields = [
        {
            "name": "image",
    ...
    def display_text(self, values=None):
        ...
        return (
            "wait for captured image: "
            f"{width}x{height}, {timeout:g}s, "
            f"{detection_title}, {match_title}"
        )
    ...
    def execute(
        ...
        check_interval = normalize_float(
            values.get(
                "check_interval",
                0.25,
            ),
        ...
        result = wait_for_captured_image(
            image_value,
            position_type,
            search_region,
            confidence,
