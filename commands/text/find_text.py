...
def build_find_text_preview(values):
    target_variable = str(values.get("target_variable", "") or "").strip() or "found_index"
    ...
    return {
        "value": f"{target_variable} = find index of {find_text} in {text_value}",
        "status": "info",
    }


class FindTextCommand(MacroCommand):
    id = "text.find"
    title = "Find Text"
    category = TextCategory
    icon = "mc:f02f"
    description = "Find text position inside a string."
    ...
    fields = [
        ...
        },
        {
            ...
            "required": False,
            "transient": True,
            "compute_value": build_find_text_preview,
        },
    ]
    ...
    def display_text(self, values=None):
        ...
        return f"find text -> {target_variable}"
    ...
    def execute(self, values=None, runtime=None):
        ...
        result_index = str("" if text_value is None else text_value).find(find_text)

        runtime.vars.add(target_variable)
        variable_type = runtime.vars.type_of(target_variable)
        final_value = runtime.helper.convert_variable_value(result_index, variable_type)
...
def register_macro(registry):
    registry.register(FindTextCommand)
